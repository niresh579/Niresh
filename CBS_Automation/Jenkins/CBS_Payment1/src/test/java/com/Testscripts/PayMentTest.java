...
public class PayMentTest extends BaseTest {
	...
	public void payment() throws Throwable {
		...
		test = extent.startTest("Community Negative Payment Test");
		...
		BaseTest.typeData(l.getMatriID(), BaseTest.getExcelData("Login", 2, 1));
...
// AD Pages
		...
		b.typeData(p.getDcardNumber(),b.getExcelData("Login", 2, 3));
		
		Thread.sleep(3000);
		b.typeData(p.getDexpreMnth(),b.getExcelData("Login", 2, 4));
		
		Thread.sleep(3000);
		b.typeData(p.getDexpreYr(),b.getExcelData("Login", 2, 5));
		
		Thread.sleep(3000);
		b.typeData(p.getDCvv(),b.getExcelData("Login", 2, 6));
	...
	//			BaseTest.reportflush2();
		...
		//		BaseTest.reportflush1(domain, error, expt);
				...
				String status1 = Integer.toString(a2);

				BaseTest.setExcelData("SMS", 5, 0, status1);
				
				
				
			}
			}else if (result.getStatus() == 1) {
				String pass = BaseTest.getExcelData("SMS", 5, 0);
				if (pass.equals("2")) {
					BaseTest.sendVFSms3();
